...
const CheckoutScreen = ({ navigation, route }: NativeStackScreenProps<StackParams, 'Checkout'>) => {
  ...
  const [additionalDriverIdentification, setAdditionalDriverIdentification] = useState('')
  const [additionalDriverDrivingLicence, setAdditionalDriverDrivingLicence] = useState('')
  
  const [userIdentificationRequired, setUserIdentificationRequired] = useState(false)
  const [userDrivingLicenceRequired, setUserDrivingLicenceRequired] = useState(false)
  const [additionalDriverIdentificationRequired, setAdditionalDriverIdentificationRequired] = useState(false)
  const [additionalDriverDrivingLicenceRequired, setAdditionalDriverDrivingLicenceRequired] = useState(false)
  ...
  const userIdentificationRef = useRef<ReactTextInput>(null)
  const userDrivingLicenceRef = useRef<ReactTextInput>(null)
  const additionalDriverIdentificationRef = useRef<ReactTextInput>(null)
  const additionalDriverDrivingLicenceRef = useRef<ReactTextInput>(null)
  ...
  const _init = async () => {
    try {
      ...
      if (!_authenticated) {
        ...
        if (userIdentificationRef.current) {
          userIdentificationRef.current.clear()
        }
        if (userDrivingLicenceRef.current) {
          userDrivingLicenceRef.current.clear()
        }
      }
      ...
      setAdditionalDriverIdentification('')
      setAdditionalDriverDrivingLicence('')
      ...
      if (additionalDriverIdentificationRef.current) {
        additionalDriverIdentificationRef.current.clear()
      }
      if (additionalDriverDrivingLicenceRef.current) {
        additionalDriverDrivingLicenceRef.current.clear()
      }
      ...
      setUserIdentificationRequired(false)
      setUserDrivingLicenceRequired(false)
  ...
  const validateUserIdentification = () => {
    const valid = userIdentification !== ''
    setUserIdentificationRequired(!valid)
    setError(!valid)
    return valid
  }
  
  const validateUserDrivingLicence = () => {
    const valid = userDrivingLicence !== ''
    setUserDrivingLicenceRequired(!valid)
    setError(!valid)
    return valid
  }
  
  const _validateAdditionalDriverIdentification = () => {
    const valid = additionalDriverIdentification !== ''
    setAdditionalDriverIdentificationRequired(!valid)
    setError(!valid)
    return valid
  }
  
  const _validateAdditionalDriverDrivingLicence = () => {
    const valid = additionalDriverDrivingLicence !== ''
    setAdditionalDriverDrivingLicenceRequired(!valid)
    setError(!valid)
    return valid
  }


  const onChangeUserIdentification = (text: string) => {
    setUserIdentification(text)
    setUserIdentificationRequired(false)
    setError(false)
  }
  
  const onChangeUserDrivingLicence = (text: string) => {
    setUserDrivingLicence(text)
    setUserDrivingLicenceRequired(false)
    setError(false)
  }
  
  const onChangeAdditionalDriverIdentification = (text: string) => {
    setAdditionalDriverIdentification(text)
    setAdditionalDriverIdentificationRequired(false)
    setError(false)
  }
  
  const onChangeAdditionalDriverDrivingLicence = (text: string) => {
    setAdditionalDriverDrivingLicence(text)
    setAdditionalDriverDrivingLicenceRequired(false)
    setError(false)
  }
  ...
  const handleCheckout = async () => {
    try {
      ...
      if (!authenticated) {
        ...
        userIdentificationRef?.current?.blur()
        userDrivingLicenceRef?.current?.blur()


        const userIdentificationValid = validateUserIdentification()
        if (!userIdentificationValid) {
          return
        }
        const userDrivingLicenceValid = validateUserDrivingLicence()
        if (!userDrivingLicenceValid) {
          return
        }
      ...
      const additionalDriverIdentificationValid = _validateAdditionalDriverIdentification()
      if (!additionalDriverIdentificationValid) {
        return
      }
      const additionalDriverDrivingLicenceValid = _validateAdditionalDriverDrivingLicence()
      if (!additionalDriverDrivingLicenceValid) {
        return
      }
      ...
      if (adRequired && additionalDriver && addtionalDriverBirthDate) {
        _additionalDriver = {
          ...
          userIdentification:additionalDriverIdentification
        }
      }
                ...
                <View style={styles.contentContainer}>
                  ...
                  {!authenticated && (
                    <View style={styles.section}>
                      ...
                      <TextInput
                          ...
                          style={styles.component}
                          label={i18n.t('IDENTIFICATION_NUMBER')}
                          value={userIdentification}
                          error={userIdentificationRequired}
                          helperText={(userIdentificationRequired && i18n.t('REQUIRED')) || ''}
                          onChangeText={onChangeUserIdentification}
                          backgroundColor="#fbfbfb"
                        />

                        <TextInput
                          ref={userDrivingLicenceRef}
                          style={styles.component}
                          label={i18n.t('DRIVING_LICENCE')}
                          value={userDrivingLicence}
                          error={userDrivingLicenceRequired}
                          helperText={(userDrivingLicenceRequired && i18n.t('REQUIRED')) || ''}
                          onChangeText={onChangeUserDrivingLicence}
                          backgroundColor="#fbfbfb"
                        />
                  ...
                  {(adManuallyChecked && additionalDriver) && (
                    <View style={styles.section}>
                      ...
                      <TextInput
                        ref={additionalDriverIdentificationRef}
                        style={styles.component}
                        label={i18n.t('IDENTIFICATION_NUMBER')}
                        value={additionalDriverIdentification}
                        error={adRequired && additionalDriverIdentificationRequired}
                        helperText={(adRequired && additionalDriverIdentificationRequired && i18n.t('REQUIRED')) || ''}
                        onChangeText={(text: string) => {
                          setAdditionalDriverIdentification(text)
                          setAdditionalDriverIdentificationRequired(false)
                          setError(false)
                      ...
                      <TextInput
                        ref={additionalDriverDrivingLicenceRef}
                        style={styles.component}
                        label={i18n.t('DRIVING_LICENCE')}
                        value={additionalDriverDrivingLicence}
                        error={adRequired && additionalDriverDrivingLicenceRequired}
                        helperText={(adRequired && additionalDriverDrivingLicenceRequired && i18n.t('REQUIRED')) || ''}
                        onChangeText={(text: string) => {
                          setAdditionalDriverDrivingLicence(text)
                          setAdditionalDriverDrivingLicenceRequired(false)
                          setError(false)
